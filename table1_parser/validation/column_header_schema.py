...
def validate_column_header_schema(schema: ColumnHeaderSchema) -> ColumnHeaderSchema:
    ...
    relationship_ids = [relationship.relationship_id for relationship in schema.relationships]
    for label, values in {
    ...
    for leaf in schema.leaves:
        if leaf.col_idx >= schema.n_cols:
            raise ValueError(f"leaf_col_idx_out_of_bounds: {leaf.leaf_id}: {leaf.col_idx}")
        missing_evidence = sorted(set(leaf.evidence_ids) - evidence_id_set)
        if missing_evidence:
            raise ValueError(f"leaf_references_unknown_evidence: {leaf.leaf_id}: {missing_evidence}")
    ...
    for group in schema.groups:
        ...
        if leaf_cols != list(range(group.col_start, group.col_end + 1)):
            raise ValueError(f"group_leaf_columns_not_contiguous: {group.group_id}: {leaf_cols}")
        if any(col_idx >= schema.n_cols for col_idx in leaf_cols):
            raise ValueError(f"group_col_idx_out_of_bounds: {group.group_id}: {leaf_cols}")
        missing_evidence = sorted(set(group.evidence_ids) - evidence_id_set)
        if missing_evidence:
            raise ValueError(f"group_references_unknown_evidence: {group.group_id}: {missing_evidence}")

    for relationship in schema.relationships:
        if relationship.parent_group_id not in group_by_id:
            raise ValueError(f"relationship_unknown_parent_group: {relationship.relationship_id}")
        if relationship.child_leaf_id not in leaf_by_id:
            raise ValueError(f"relationship_unknown_child_leaf: {relationship.relationship_id}")
        ...
        if relationship.leaf_col_idx != leaf.col_idx:
            raise ValueError(f"relationship_leaf_col_mismatch: {relationship.relationship_id}")
        if relationship.leaf_col_idx not in group.leaf_col_indices:
            raise ValueError(f"relationship_leaf_not_in_parent_group: {relationship.relationship_id}")
        missing_evidence = sorted(set(relationship.evidence_ids) - evidence_id_set)
        if missing_evidence:
            raise ValueError(f"relationship_references_unknown_evidence: {relationship.relationship_id}: {missing_evidence}")
