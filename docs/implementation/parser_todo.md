...
9. [ ] Clean up benign PDF text artifacts cautiously.
   ...
   Bibliographic reference follow-up: `paper_bibliography.json` now preserves the paper's own numbered bibliography entries and links numeric row-label study/source markers to those entries when no local table-note definition exists. The footnote linker suppresses clustered citation-like numeric row-label markers from table-footnote link counts, while the original marker evidence remains visible in `cell_text_annotations.json` and linked in `paper_bibliography.json`.
