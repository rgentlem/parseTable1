...
9. [ ] Clean up benign PDF text artifacts cautiously.
   ...
   Recent math/unit update: numeric superscripts and subscripts in expressions like `10^9`, `10^6`, `m^2`, `kg/m^2`, `CO₂`, `I²`, and `×10^9/L` are rejected before `FootnoteAnchor` creation, and multi-letter subscript words such as `P_Begg`/`P_Egger` are suppressed as non-footnote annotations. They remain visible in `cell_text_annotations.json`; `paper_footnotes.json` records the suppression counts in `math_unit_anchor_suppression_count` and `word_like_subscript_anchor_suppression_count`.
